...
const Utils = require('@bonjourjohn/utils').Objects;

module.exports = class Model {
  constructor(schema, params = null) {
    Utils.inspectSchema(schema);
    this.schema = schema;
  ...
  load(params) {
    ...
    Utils.matchFieldsAndTypes(params, this.schema);
  ...
  loadParams(params, target = null, schema = null) {
    ...
    for (const key of Object.keys(schema)) {
      ...
      if (schema[key].constructor.name === "Object") {
        ...
        this.loadParams(Utils.getValueAt(params, key), target[key], schema[key]);
        ...
        target[key] = Utils.getValueAt(params, key);
      }
    }
  }
  ...
  get(key = null) {
    if (key) {
      if (!Utils.hasProperty(this.schema, key)) {
        throw new Error("Object " + this.constructor.name + " has no property " + key);
      } else {
        return Utils.getValueAt(this, key);
      }
    }

    let object = Object.assign({}, this);
    object = Utils.keepProperties(object, Object.keys(this.schema));
    Utils.removeEmptyProperties(object);
